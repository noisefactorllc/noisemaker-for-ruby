...
run_pixel = lambda do |ctx, out|
  ...
  main__void = lambda do
    ...
    _u = rt.swizzle(here, 'rg')
    dt = rt.binary('*', rt.component_wise('clamp', _u_speed, rt.f(0), rt.f(200)), rt.f(0.0001), 1, 'float')
    backUv = rt.component_wise('clamp', rt.binary('-', uv, rt.binary('*', _u, dt, 2, 'float'), 2, 'float'), rt.construct(2, rt.f(0)), rt.construct(2, rt.f(1)))
    advected = sampleBilinear__vec2_ivec2.call(backUv, texSize)
