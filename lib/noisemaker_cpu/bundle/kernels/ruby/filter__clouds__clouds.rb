# Generated by NoisemakerCpu::Transpiler - do not edit.
run_pixel = lambda do |ctx, out|
  rt = ctx.rt
  u = ctx.uniforms
  g = {}
  mod289__vec3 = mod289__vec2 = permute__vec3 = simplex2d__vec2 = cloudNoise__vec2_float_int_float_float = main__void = nil
  _retc = nil
  _u_inputTex = ctx.texture_binding('inputTex')
  _u_tileOffset = u.key?('tileOffset') ? u['tileOffset'] : rt.construct(2, 0.0)
  _u_fullResolution = u.key?('fullResolution') ? u['fullResolution'] : rt.construct(2, 0.0)
  _u_seed = u.key?('seed') ? u['seed'] : rt.f(0.0)
  _u_scale = u.key?('scale') ? u['scale'] : rt.f(0.0)
  _u_speed = u.key?('speed') ? u['speed'] : 0
  _u_time = u.key?('time') ? u['time'] : rt.f(0.0)
  g['fragColor'] = rt.construct(4, 0.0)
  g['TAU'] = rt.f(6.2831853071800001)
  mod289__vec3 = lambda do |x|
    x = rt.copy(x, 'float')
    return rt.binary('-', x, rt.binary('*', rt.component_wise('floor', rt.binary('*', x, rt.binary('/', rt.f(1), rt.f(289), 1, 'float'), 3, 'float')), rt.f(289), 3, 'float'), 3, 'float')
  end
  mod289__vec2 = lambda do |x|
    x = rt.copy(x, 'float')
    return rt.binary('-', x, rt.binary('*', rt.component_wise('floor', rt.binary('*', x, rt.binary('/', rt.f(1), rt.f(289), 1, 'float'), 2, 'float')), rt.f(289), 2, 'float'), 2, 'float')
  end
  permute__vec3 = lambda do |x|
    x = rt.copy(x, 'float')
    return mod289__vec3.call(rt.binary('*', rt.binary('+', rt.binary('*', x, rt.f(34), 3, 'float'), rt.f(1), 3, 'float'), x, 3, 'float'))
  end
  simplex2d__vec2 = lambda do |v|
    v = rt.copy(v, 'float')
    _C = nil; _g = nil; a0 = nil; h = nil; i = nil; i1 = nil; m = nil; ox = nil; p = nil; x = nil; x0 = nil; x12 = nil
    _C = rt.construct(4, rt.construct(4, rt.f(0.211324865405187), rt.f(0.36602540378443899), rt.unary('-', rt.f(0.57735026918962595)), rt.f(0.024390243902439001)))
    i = rt.construct(2, rt.component_wise('floor', rt.binary('+', v, rt.dot(v, rt.swizzle(_C, 'yy')), 2, 'float')))
    x0 = rt.construct(2, rt.binary('+', rt.binary('-', v, i, 2, 'float'), rt.dot(i, rt.swizzle(_C, 'xx')), 2, 'float'))
    i1 = rt.construct(2, (rt.bool(rt.binary('>', rt.swizzle(x0, 'x'), rt.swizzle(x0, 'y'))) ? (rt.construct(2, rt.f(1), rt.f(0))) : (rt.construct(2, rt.f(0), rt.f(1)))))
    x12 = rt.construct(4, rt.binary('+', rt.swizzle(x0, 'xyxy'), rt.swizzle(_C, 'xxzz'), 4, 'float'))
    x12 = rt.assign_swizzle(x12, 'xy', rt.binary('-', rt.swizzle(x12, 'xy'), i1, 2, 'float'))
    i.replace((mod289__vec2.call(i)).map { |c| rt.f32(c) })
    p = rt.construct(3, permute__vec3.call(rt.binary('+', rt.binary('+', permute__vec3.call(rt.binary('+', rt.swizzle(i, 'y'), rt.construct(3, rt.f(0), rt.swizzle(i1, 'y'), rt.f(1)), 3, 'float')), rt.swizzle(i, 'x'), 3, 'float'), rt.construct(3, rt.f(0), rt.swizzle(i1, 'x'), rt.f(1)), 3, 'float')))
    m = rt.construct(3, rt.component_wise('max', rt.binary('-', rt.f(0.5), rt.construct(3, rt.dot(x0, x0), rt.dot(rt.swizzle(x12, 'xy'), rt.swizzle(x12, 'xy')), rt.dot(rt.swizzle(x12, 'zw'), rt.swizzle(x12, 'zw'))), 3, 'float'), rt.f(0)))
    m.replace((rt.binary('*', m, m, 3, 'float')).map { |c| rt.f32(c) })
    m.replace((rt.binary('*', m, m, 3, 'float')).map { |c| rt.f32(c) })
    x = rt.construct(3, rt.binary('-', rt.binary('*', rt.f(2), rt.component_wise('fract', rt.binary('*', p, rt.swizzle(_C, 'www'), 3, 'float')), 3, 'float'), rt.f(1), 3, 'float'))
    h = rt.construct(3, rt.binary('-', rt.component_wise('abs', x), rt.f(0.5), 3, 'float'))
    ox = rt.construct(3, rt.component_wise('floor', rt.binary('+', x, rt.f(0.5), 3, 'float')))
    a0 = rt.construct(3, rt.binary('-', x, ox, 3, 'float'))
    m.replace((rt.binary('*', m, rt.binary('-', rt.f(1.79284291400159), rt.binary('*', rt.f(0.85373472095313996), rt.binary('+', rt.binary('*', a0, a0, 3, 'float'), rt.binary('*', h, h, 3, 'float'), 3, 'float'), 3, 'float'), 3, 'float'), 3, 'float')).map { |c| rt.f32(c) })
    _g = rt.construct(3, 0.0)
    _g = rt.assign_swizzle(_g, 'x', rt.binary('+', rt.binary('*', rt.swizzle(a0, 'x'), rt.swizzle(x0, 'x'), 1, 'float'), rt.binary('*', rt.swizzle(h, 'x'), rt.swizzle(x0, 'y'), 1, 'float'), 1, 'float'))
    _g = rt.assign_swizzle(_g, 'yz', rt.binary('+', rt.binary('*', rt.swizzle(a0, 'yz'), rt.swizzle(x12, 'xz'), 2, 'float'), rt.binary('*', rt.swizzle(h, 'yz'), rt.swizzle(x12, 'yw'), 2, 'float'), 2, 'float'))
    return rt.binary('*', rt.f(130), rt.dot(m, _g), 1, 'float')
  end
  cloudNoise__vec2_float_int_float_float = lambda do |uv, baseFreq, octaves, animPhase, animSpeed|
    uv = rt.copy(uv, 'float')
    _for0_first = nil; accum = nil; amp = nil; freq = nil; i = nil; n = nil; octavePhase = nil; octaveRadius = nil; timeOffset = nil; totalAmp = nil
    accum = rt.f(0)
    totalAmp = rt.f(0)
    i = rt.i(0)
    _for0_first = true
    (0..1048575).each do |_for0|
      unless _for0_first
        i = rt.binary('+', i, rt.i(1), 1, 'int')
      end
      _for0_first = false
      unless rt.bool(rt.binary('<', i, rt.i(8)))
        break
      end
      if rt.bool(rt.binary('>=', i, octaves))
        break
      end
      freq = rt.binary('*', baseFreq, rt.component_wise('pow', rt.f(2), rt.construct(1, i)), 1, 'float')
      amp = rt.binary('/', rt.f(1), rt.component_wise('pow', rt.f(2), rt.construct(1, i)), 1, 'float')
      octavePhase = rt.binary('*', rt.construct(1, i), rt.f(2.1299999999999999), 1, 'float')
      octaveRadius = rt.binary('*', rt.binary('+', rt.f(0.25), rt.binary('*', rt.construct(1, i), rt.f(0.080000000000000002), 1, 'float'), 1, 'float'), animSpeed, 1, 'float')
      timeOffset = rt.construct(2, rt.binary('*', rt.binary('-', rt.construct(2, rt.component_wise('cos', rt.binary('+', animPhase, octavePhase, 1, 'float')), rt.component_wise('sin', rt.binary('+', animPhase, octavePhase, 1, 'float'))), rt.construct(2, rt.component_wise('cos', octavePhase), rt.component_wise('sin', octavePhase)), 2, 'float'), octaveRadius, 2, 'float'))
      n = simplex2d__vec2.call(rt.binary('+', rt.binary('+', rt.binary('*', uv, freq, 2, 'float'), rt.construct(2, rt.binary('*', rt.construct(1, i), rt.f(37), 1, 'float'), rt.binary('*', rt.construct(1, i), rt.f(53), 1, 'float')), 2, 'float'), timeOffset, 2, 'float'))
      n = rt.binary('+', rt.binary('*', n, rt.f(0.5), 1, 'float'), rt.f(0.5), 1, 'float')
      accum = rt.binary('+', accum, rt.binary('*', n, amp, 1, 'float'), 1, 'float')
      totalAmp = rt.binary('+', totalAmp, amp, 1, 'float')
    end
    return rt.binary('/', accum, totalAmp, 1, 'float')
  end
  main__void = lambda do
    animPhase = nil; animSpeed = nil; aspect = nil; cloud = nil; cloudBrightness = nil; cloudDepth = nil; cloudMask = nil; cloudUV = nil; globalCoord = nil; globalUV = nil; inputColor = nil; resolution = nil; result = nil; seedOffset = nil; shadow = nil; shadowCloud = nil; shadowDist = nil; shadowMask = nil; shadowOffset = nil; shadowUV = nil; texSize = nil; tileDims = nil; uv = nil
    globalCoord = rt.construct(2, rt.binary('+', rt.swizzle(ctx.frag_coord, 'xy'), _u_tileOffset, 2, 'float'))
    texSize = rt.texture_size(_u_inputTex)
    tileDims = rt.construct(2, rt.construct(2, texSize))
    resolution = rt.construct(2, (rt.bool(rt.binary('>', rt.swizzle(_u_fullResolution, 'x'), rt.f(0))) ? (_u_fullResolution) : (tileDims)))
    uv = rt.construct(2, rt.binary('/', rt.swizzle(ctx.frag_coord, 'xy'), tileDims, 2, 'float'))
    globalUV = rt.construct(2, rt.binary('/', rt.binary('+', rt.swizzle(ctx.frag_coord, 'xy'), _u_tileOffset, 2, 'float'), resolution, 2, 'float'))
    inputColor = rt.construct(4, rt.texture(_u_inputTex, uv))
    aspect = rt.binary('/', rt.swizzle(_u_fullResolution, 'x'), rt.swizzle(_u_fullResolution, 'y'), 1, 'float')
    seedOffset = rt.construct(2, rt.construct(2, rt.binary('*', _u_seed, rt.f(17.309999999999999), 1, 'float'), rt.binary('*', _u_seed, rt.f(23.710000000000001), 1, 'float')))
    animPhase = rt.binary('*', rt.binary('*', _u_time, g['TAU'], 1, 'float'), rt.construct(1, _u_speed), 1, 'float')
    animSpeed = rt.construct(1, _u_speed)
    cloudUV = rt.construct(2, rt.binary('+', rt.binary('/', rt.binary('*', globalUV, rt.construct(2, aspect, rt.f(1)), 2, 'float'), _u_scale, 2, 'float'), seedOffset, 2, 'float'))
    cloud = cloudNoise__vec2_float_int_float_float.call(cloudUV, rt.f(1), rt.i(7), animPhase, animSpeed)
    cloudMask = rt.component_wise('smoothstep', rt.f(0.45000000000000001), rt.f(0.65000000000000002), cloud)
    cloudDepth = rt.component_wise('smoothstep', rt.f(0.45000000000000001), rt.f(0.84999999999999998), cloud)
    cloudBrightness = rt.component_wise('mix', rt.f(0.75), rt.f(1), cloudDepth)
    shadowDist = rt.binary('*', rt.component_wise('min', rt.swizzle(resolution, 'x'), rt.swizzle(resolution, 'y')), rt.f(0.0080000000000000002), 1, 'float')
    shadowOffset = rt.construct(2, rt.binary('/', rt.construct(2, rt.unary('-', shadowDist), shadowDist), resolution, 2, 'float'))
    shadowUV = rt.construct(2, rt.binary('+', rt.binary('/', rt.binary('*', rt.binary('+', globalUV, shadowOffset, 2, 'float'), rt.construct(2, aspect, rt.f(1)), 2, 'float'), _u_scale, 2, 'float'), seedOffset, 2, 'float'))
    shadowCloud = cloudNoise__vec2_float_int_float_float.call(shadowUV, rt.f(1), rt.i(7), animPhase, animSpeed)
    shadowMask = rt.component_wise('smoothstep', rt.f(0.45000000000000001), rt.f(0.65000000000000002), shadowCloud)
    shadow = rt.binary('*', rt.component_wise('max', rt.binary('-', shadowMask, cloudMask, 1, 'float'), rt.f(0)), rt.f(0.5), 1, 'float')
    result = rt.construct(3, rt.binary('*', rt.swizzle(inputColor, 'rgb'), rt.binary('-', rt.f(1), shadow, 1, 'float'), 3, 'float'))
    result.replace((rt.component_wise('mix', result, rt.construct(3, cloudBrightness), cloudMask)).map { |c| rt.f32(c) })
    g['fragColor'].replace((rt.construct(4, result, rt.swizzle(inputColor, 'a'))).map { |c| rt.f32(c) })
  end
  main__void.call
  c = g['fragColor']
  out[0] = rt.f32(c[0]); out[1] = rt.f32(c[1]); out[2] = rt.f32(c[2]); out[3] = rt.f32(c[3])
end
{ kernel: run_pixel, uses_derivatives: false }
